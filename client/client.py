...
from common.crypto import * #加密相关工具
import common.types as types #定义了通用数据结构
import common.codec as codec #编码解码工具
import common.errors as errors # 定义了错误类型
...
class Client:
    ...
    def register(self) -> None:
        ...
        req = types.RegisterRequest(
            self._username, self._auth_secret, codec.encode(log))
    ...
    def update_public_profile(self, values: t.Dict[str, t.Any]) -> None:
        ...
        >>> link_client_server(alice, server)
        >>> alice.register()
        >>> profile = {'bio': 'I like computer security and cryptography', 'location': 'York', 'camera': 'mobile phone'}
        ...
        req = types.UpdatePublicProfileRequest(
            username=self._username,                     # 当前用户名
            token=self._server_session_token,            # 当前会话令牌
        ...
        if resp.error is None:
            # 更新成功，无需进一步操作
            return
        elif resp.error == types.Errcode.INVALID_TOKEN:
    ...
    def get_friend_public_profile(self, friend_username: str) -> types.PublicProfile:
        ...
        if resp.error is None:
            ...
            raise errors.InvalidTokenError()
        elif resp.error == types.Errcode.USER_NOT_FOUND:
            raise errors.UserNotFoundError(friend_username)
        else:
            # 处理未知错误
            raise Exception(resp)
    ...
    def _synchronize(self):
        ...
        if self._last_log_number == 0:
            raise errors.SynchronizationError(
                errors.MissingRegistrationError())
